...
import {Magasin} from "../../../store/entities/gestock.entity";
...
export class MagasinComponent implements OnInit {
    ...
    add(magasinValue: any) {
        if (magasinValue === null) {
            this.modal = 'ajouter';
            this.magasin = {}
        } else {
            this.magasin = magasinValue;
            this.modal = 'modifier';
        }
        this.display = true;
    }
    ...
    deleteElement(magasinToDelete: Magasin) {
        this.confirmationService.confirm({
            header: 'Confirmation',
            message: 'Etes-vous sûr de vouloir supprimer ?',
            accept: () => {
                if (magasinToDelete === null) {
                    return;
                } else {

                    if (magasinToDelete.id != null) {
                        this.chrgmt = true;
                        this.magasinService.delete(magasinToDelete.id).subscribe(
                            async () => {
                                this.loadAll().then(()=>{
    ...
    save(editForm: NgForm) {
        this.magasin.societyId = JSON.parse(this.tokenStorage.getsociety()!);
        if (!this.ifExist()) {
            this.display = false;
            this.chrgmt = true;

                if (this.magasin?.id) {
                        this.magasinService.update(this.magasin).subscribe(
                            () => {
                                this.loadAll().then(()=>{
                        ...
                        this.magasinService.save(this.magasin).subscribe(
                            () => {
                                this.loadAll().then(() => {
            ...
            this.showMessage('error', 'ENREGISTREMENT', 'Un magasin portant le même nom existe déjà !');
        }
    ...
    ifExist(): boolean {
        if (this.magasin.id) {
            return this.magasins.some(
                value =>
                    value.id !== this.magasin.id &&
                    value.nom === this.magasin.nom
            );
        } else {
            return this.magasins.some(value => value.nom === this.magasin.nom);
        }
    }
            ...
            items: [
                {
                    label: 'Supprimer',
                    icon: 'pi pi-times',
                    disabled:!this.droits.includes('SUPPRIMER_MAGASIN'),
                    command: ($event: any) => {
                ...
                },
                {
                    label: 'Modifier',
                    icon: 'pi pi-pencil',
                    disabled:!this.droits.includes('MODIFIER_MAGASIN'),
                    command: ($event: any) => {
