...
export class TransfertlistComponent implements OnInit {
    ...
    list_entrees_stockMagasin: Entree_stock[] = [];
    ...
    loadall(){
        ...
        this.entreeStockService.findbysociety(JSON.parse(this.tokenStorage.getsociety()!)).subscribe(
            (res) => {
                ...
                this.list_entrees_stockMagasin = res.payload.filter(item => item.fournisseurId === null);
    ...
    DeleteEntree(selectedItem: any) {
        ...
        this.confirmationService.confirm({
            ...
            message: 'Voulez-vous vraiment Annuler le transfert? les quantités seront remises à jour dans haque magasins concernés!',
            accept: () => {
    ...
    UpdateEntreecontent(selectedItem: any) {
        ...
        // @ts-ignore
        if(this.produitselected.ligneMagasinDTOS?.length>0 && this.magasinselected && this.produitselected){
    ...
    detailsTransfert(selectedItem: any) {
        this.entree_stock = selectedItem;
